package com.example.dllo.lexuebdemo.teacher.customview;
...
public class DragGridView extends GridView implements AdapterView.OnItemLongClickListener {
    ...
    public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        mY = mWindowY - view.getTop() - this.getTop() - 70;
    ...
    private void initWindow() {
        if (dragView == null) {
            //TODO 拖拽view
            dragView = View.inflate(getContext(), R.layout.item_pop_window_teacher_taglist, null);
            TextView tv_text = (TextView) dragView.findViewById(R.id.item_pop_window_tv);
            //将文字设置为点击时间中view的文字
            tv_text.setText(((TextView) view.findViewById(R.id.item_pop_window_tv)).getText());
        }
        if (layoutParams == null) {
            ...
            //移动的时候纵轴偏移有点偏差
            layoutParams.y = view.getTop() + this.getTop() + 80;  //悬浮窗Y的位置
    ...
    private void updateWindow(MotionEvent ev) {
        if (mode == MODE_DRAG) {
            ...
            if (dropPosition == tempPosition || dropPosition == GridView.INVALID_POSITION
                    || dropPosition == 0 || dropPosition == childCount-1) {
                return;
            }
